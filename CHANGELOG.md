...
- **生成速度不再出现离谱数字**（真机见过 6996 tok/s）：口径回到「输出 token ÷ 整回合耗时」，
  和旁边的「耗时」是同一个数，你可以自己验算。带工具的回合会把执行工具的时间算进去，
  所以显示个位数是正常的；思考模型的数字包含思考阶段的速度，不再只算正文那一段。
- **不再出现写着 `null` 的思考卡**：有些服务商在一次性返回整段回答时会把「思考内容」字段
  显式回成 `null`，以前会被当成一段思考渲染出来。

### 文档

- 开源署名补齐三家：Agora（GPL-3.0，流式观感与列表收口的参照）、opencode（MIT，上下文压缩
  阈值机制）、内置 `skill-creator` 技能保留它自己的 Apache-2.0 许可文本与上游 commit 记录。

## [1.0.21] — 2026-09-25

### 修复

- **生成速度不再出现 600 tok/s 这种数**：上一版算的是「总耗时 − 第一个字」，遇到一次性
  吐完的回答，分母接近 0，速度就被除上天了。现在按每一段正文真正在流的时间累加来算，
  测不出有效窗口时退回总耗时，绝不除以零头；带工具的回合也不再算成「模型一直在写字」。
- **输入栏的麦克风不再凭空出现**：没在设置里配过语音输入，就不该有那颗钮 —— 现在和
  原版一致，只有选中了语音识别服务才显示（选系统识别时，才看本机有没有识别器）。
- 工作区里 html / markdown 的**渲染预览补齐**：聊天输入栏打开的那个文件预览以前还是
  纯文本，现在和「管理工作区」详情页一致。

### 改进

- **模型不能改它没读过的文件**：`workspace_edit_file` 现在要求这条对话里先读过那个文件，
  且读过之后文件没再变过（变了会要求重读）—— 避免它凭猜把你在沙箱里手改的内容覆盖掉，
  而且以前这种覆盖还会被报成「替换成功」。
- 工具参数写错时，回给模型的是一份**能照着改**的清单（哪颗参数、什么约束、怎么补），
  并明确「什么都没执行、什么都没变」；以前只回半句 `path is required`。
- 外部内容（搜索结果、网页、工作区文件、MCP 返回）里如果夹带我们用来划边界的框架标签，
  会被降级成普通文字 —— 堵住「让模型读一段文本就冒充系统指令」这条路。
- 加了一张**系统提示词基线网**：提示词的任何一句文案、任何一处顺序或门控变化，都会让
  这条测试变红，必须显式更新基线才能合进去。
- 生成不再跟着界面生死：会话改由应用级容器持有，Activity 被销毁重建（旋转、主题切换、
  系统回收）不再打断正在输出的回答，回到会话直接接上。进程被杀仍然不能续跑。
