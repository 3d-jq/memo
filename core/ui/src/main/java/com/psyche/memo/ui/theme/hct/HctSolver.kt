...
/**
 * Port of `hct/src/hct_solver.dart` (material_color_utilities 0.13.0): maps
 * hue/chroma/tone back into the sRGB gamut. [CRITICAL_PLANES] is the upstream
 * table copied verbatim — it is looked up by index during the bisection, so
 * deriving it numerically instead would risk a last-ULP difference flipping a
 * comparison.
 */
internal object HctSolver {
    ...
    /** Sanitizes a small enough angle in radians to a coterminal angle in [0, 2pi). */
    private fun sanitizeRadians(angle: Double): Double = MathUtils.sanitizeRadians(angle)

    /**
     * Delinearizes an RGB component without quantizing to 8 bits, so the result
     * can be compared against the plane table.
     */
    ...
    /** Hue of [linrgb] in CAM16, in radians. */
    ...
    /** Solves the lerp equation: t such that lerp([source], [target], t) == [mid]. */
    ...
    /** Intersects the segment [source]→[target] with the plane `axis == coordinate`. */
    ...
    /**
     * Returns the nth possible vertex of the polygonal intersection of the plane
     * `Y == [y]` with the RGB cube, or `[-1, -1, -1]` when it lies outside.
     */
    private fun nthVertex(y: Double, n: Int): DoubleArray {
        ...
        val unbounded = doubleArrayOf(-1.0, -1.0, -1.0)
        return if (n < 4) {
            val g = coordA
            val b = coordB
            val r = (y - g * kG - b * kB) / kR
            if (isBounded(r)) doubleArrayOf(r, g, b) else unbounded
        } else if (n < 8) {
            val b = coordA
            val r = coordB
            val g = (y - r * kR - b * kB) / kG
            if (isBounded(g)) doubleArrayOf(r, g, b) else unbounded
        } else {
            val r = coordA
            val g = coordB
            val b = (y - r * kR - g * kG) / kB
            if (isBounded(b)) doubleArrayOf(r, g, b) else unbounded
        }
    }

    /** Finds the segment on plane `Y == [y]` that contains [targetHue]. */
    ...
    /** Finds the color with the given Y and hue on the boundary of the cube. */
    ...
    /**
     * Finds a color with the given hue, chroma and Y. Returns 0 when none exists
    ...
    private fun findResultByJ(hueRadians: Double, chroma: Double, y: Double): Int {
        ...
        val viewingConditions = ViewingConditions.SRGB
        val tInnerCoeff = 1.0 / (1.64 - 0.29.pow(viewingConditions.backgroundYTowhitePointY)).pow(0.73)
        ...
        for (iterationRound in 0 until 5) {
            ...
            val ac = viewingConditions.aw * jNormalized.pow(1.0 / viewingConditions.c / viewingConditions.z)
    ...
    /**
     * Finds an sRGB color with the given hue, chroma and L*. If all three cannot
     * be satisfied, hue and L* are honored and chroma is maximized.
     */
    fun solveToInt(hueDegrees: Double, chroma: Double, lstar: Double): Int {
        if (chroma < 0.0001 || lstar < 0.0001 || lstar > 99.9999) {
            return ColorUtils.argbFromLstar(lstar)
        }
        val sanitizedHue = MathUtils.sanitizeDegreesDouble(hueDegrees)
        val hueRadians = sanitizedHue / 180 * PI
