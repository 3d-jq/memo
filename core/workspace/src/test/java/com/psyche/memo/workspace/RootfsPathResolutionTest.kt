...
/**
 * `resolveRootfsPath` 的映射与拒绝规则 —— 上游
 * `workspace/src/test/.../RootfsPathResolutionTest.kt:12-128` 1:1 移植。
 *
 * 覆盖 bind mount「最长 target 优先」、`/workspace` → `files/`、未知绝对路径回落 rootfs
 * 内部、以及三条拒绝路径（穿越出 bind mount / 内核伪文件系统 / 目录当文件读）。
 * 上游用 Koin 注入的目录在这里直接换成测试自己创建的目录。
 */
class RootfsPathResolutionTest {
    ...
    fun unknownAbsolutePathFallsBackToRootfsInterior() {
        ...
        File(manager.linuxDir(root), "etc/hostname").writeText("memo\n")
