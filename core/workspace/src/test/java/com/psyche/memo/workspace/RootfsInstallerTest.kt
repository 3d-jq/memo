...
/**
 * tar 解包边界 —— 上游 `workspace/src/test/.../RootfsInstallerTest.kt:12-72` 1:1 移植。
 *
 * 覆盖 `extractTar` 的手写 reader 最容易错的两处：`OTHER`（如 GNU sparse）条目的数据区
 * 只能跳一次，以及目录 / 0 字节条目不能让后续 header 错位。
 */
