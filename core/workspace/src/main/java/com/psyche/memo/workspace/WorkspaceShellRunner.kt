...
/**
 * 命令执行器接口 —— 上游 `workspace` 模块 `WorkspaceShellRunner.kt:8-143` 1:1 移植。
 *
 * 两种实现：没有 rootfs 时用宿主的 [HostShellRunner]，装了 rootfs 之后用
 * [ProotShellRunner]。两者共用 [readResult] 这一套进程收尾逻辑。
 *
 * 与上游的差异只有日志：上游没有日志（应用层用 slf4j 风格封装），本工程没有那套基础设施，
 * 按 core:data / app 的惯例改用 `android.util.Log` + 文件级 [TAG]。
 */

private const val TAG = "WorkspaceShell"
...
private class StreamWriter(
    ...
    private val thread = Thread {
        try {
            ...
            Log.w(TAG, "Failed to write stdin to workspace command", e)
        }
    }.apply {
        isDaemon = true
        start()
...
private class StreamCollector(
    ...
    private val thread = Thread {
        try {
            ...
            Log.w(TAG, "Workspace command stream closed while reading", e)
        }
    }.apply {
