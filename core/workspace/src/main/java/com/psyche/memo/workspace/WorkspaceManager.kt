...
/**
 * 工作区目录布局与文件区 / rootfs 的统一入口 —— 上游 `workspace` 模块
 * `WorkspaceManager.kt:9-252` 1:1 移植。
 *
 * 目录布局（`baseDir/<root>/`）：
 * ```
 * files/  工作区文件区，rootfs 内挂在 /workspace
 * linux/  rootfs 本体（bin/sh 存在才算装好）
 * tmp/    PRoot 的 PROOT_TMP_DIR / TMPDIR
 * ```
 * 上游由 Koin 注入 `baseDir`（应用私有目录）、bind mount 列表与 shell runner；这里改成
 * 构造参数，默认 runner 仍是 [HostShellRunner]，调用方（app 模块）自己传
 * [ProotShellRunner] 即可。
 */
