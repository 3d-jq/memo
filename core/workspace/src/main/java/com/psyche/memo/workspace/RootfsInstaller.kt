...
/**
 * Rootfs 安装：下载 → 解压 → 打补丁 —— 上游 `workspace` 模块
 * `RootfsInstaller.kt:15-418` 1:1 移植。
 *
 * 解压是手写的 tar 读取器（不用 Apache Commons Compress，省一个依赖）：
 * 支持 ustar 前缀、GNU long name/link（`L`/`K`）、PAX 扩展头（`x`）与 `0/5/2/1`
 * 四种常规类型；压缩格式按扩展名选 `xz`（`.tar.xz`/`.txz`，走 [XZInputStream]）或
 * `gzip`（其余，走 [GZIPInputStream]）。
 *
 * 安全边界有两条：tar 条目路径经 [normalizeTarPath] 拒绝 `..`（[safeResolve] 再做一次
 * canonical containment），符号链接的解链点必须落在 rootfs 内。
 */
