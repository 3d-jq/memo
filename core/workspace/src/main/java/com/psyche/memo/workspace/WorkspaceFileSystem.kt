...
/**
 * 工作区文件区（`files/`）或 rootfs（`linux/`）内的文件读写 —— 上游 `workspace` 模块
 * `WorkspaceFileSystem.kt:12-214` 1:1 移植。
 *
 * 唯一的安全边界是 [resolvePath]：所有路径先 canonical 化，再要求落在 root 之内
 * （`target == root || target.startsWith(root + File.separator)`），因此 `../` 与
 * 指向 root 外的符号链接都会被 `IllegalArgumentException` 拒绝。错误文案与上游逐字一致
 * （测试按文案断言，且这些文案会透传给 LLM 工具）。
 */
