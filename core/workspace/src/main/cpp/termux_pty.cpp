// 交互式终端页的 PTY 后端 —— 上游 `workspace/src/main/cpp/termux_pty.cpp` 1:1 移植。
//
// 提供 `com.termux.termux-app:terminal-emulator` 期望的四个 JNI 符号
// （`Java_com_termux_terminal_JNI_{createSubprocess,setPtyWindowSize,waitFor,close}`）：
// AAR 里只有 Java 侧的 native 声明，.so 必须由我们提供 —— 这就是为什么 core:workspace
// 要开 externalNativeBuild，以及 app 模块要 pickFirsts `lib/*/libtermux.so`。
//
// 唯一的品牌化差异：日志 tag 用 MemoTermuxJni（上游 RikkaTermuxJni）。
#include <jni.h>
