...
/**
 * memory_prompts.dart 1:1 — built-in default prompt templates for the memory
 * system. These strings are model contracts and must NOT go through UiR
 * string resources (§16.2). Placeholders like {{conversation}} are literal.
 */
enum class MemoryPromptLang { zh, en }
...
""".trim()
    ...
    const val extractToolDefaultScopeRuleZh: String =
        "- 只对当前助手成立的信息，在 item 上加 scope=\"assistant\"；对所有场景都成立的加 scope=\"global\" 或省略"

    const val extractToolDefaultScopeRuleEn: String =
...
""".trim()
    ...
    const val introFullZh: String = "以下内容由系统提供，不是用户本轮发送的内容。"
    const val introFullEn: String =
    ...
    const val introUpdateZh: String = "以下是本次对话开始后发生的记忆更新，由系统提供。"
    const val introUpdateEn: String =
        "The following memory changes happened after this conversation started, provided by the system."

    // ── §7.2 moreHint ────────────────────────────────────────────────────────

    const val moreHintZh: String = "[更多内容请使用 memory_search_profile 查询]"
    const val moreHintEn: String =
        "[More entries exist. Use memory_search_profile to look them up.]"

    // ── §9.1 / §9.3 time helpers ─────────────────────────────────────────────

    private val weekdayAbbrev = listOf("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun")

    /** Wraps [timestamp] as `<current_time>EEE yyyy-MM-dd HH:mm:ss</current_time>` (§9.1). */
    fun formatCurrentTimeTag(timestamp: Long): String {
        val cal = java.util.Calendar.getInstance().apply { timeInMillis = timestamp }
        fun two(v: Int) = v.toString().padStart(2, '0')
        val eee = weekdayAbbrev[((cal.get(java.util.Calendar.DAY_OF_WEEK) + 5) % 7)]
        return "<current_time>$eee ${cal.get(java.util.Calendar.YEAR)}-" +
            "${two(cal.get(java.util.Calendar.MONTH) + 1)}-${two(cal.get(java.util.Calendar.DAY_OF_MONTH))} " +
            "${two(cal.get(java.util.Calendar.HOUR_OF_DAY))}:${two(cal.get(java.util.Calendar.MINUTE))}:" +
            "${two(cal.get(java.util.Calendar.SECOND))}</current_time>"
    }

    /** Returns which of `{cur_date}`, `{cur_time}`, `{cur_datetime}` occur in [systemPrompt] (§9.3). */
    fun detectTimeVariablesInSystemPrompt(systemPrompt: String): List<String> =
        listOf("{cur_date}", "{cur_time}", "{cur_datetime}").filter { systemPrompt.contains(it) }

    fun rulesFor(lang: MemoryPromptLang) = if (lang == MemoryPromptLang.zh) rulesZh else rulesEn
    fun rulesPastConversationRecallFor(lang: MemoryPromptLang) =
        if (lang == MemoryPromptLang.zh) rulesPastConversationRecallZh else rulesPastConversationRecallEn
    fun gateFor(lang: MemoryPromptLang) = if (lang == MemoryPromptLang.zh) gateZh else gateEn
    fun extractFor(lang: MemoryPromptLang) = if (lang == MemoryPromptLang.zh) extractZh else extractEn
    fun extractToolDefaultScopeRuleFor(lang: MemoryPromptLang) =
        if (lang == MemoryPromptLang.zh) extractToolDefaultScopeRuleZh else extractToolDefaultScopeRuleEn
    fun smartAddFor(lang: MemoryPromptLang) = if (lang == MemoryPromptLang.zh) smartAddZh else smartAddEn
    fun smartAddBatchFor(lang: MemoryPromptLang) =
        if (lang == MemoryPromptLang.zh) smartAddBatchZh else smartAddBatchEn
    fun profileDistillFor(lang: MemoryPromptLang) =
        if (lang == MemoryPromptLang.zh) profileDistillZh else profileDistillEn
    fun migrateFor(lang: MemoryPromptLang) = if (lang == MemoryPromptLang.zh) migrateZh else migrateEn
    fun migratePreserveFor(lang: MemoryPromptLang) =
        if (lang == MemoryPromptLang.zh) migratePreserveZh else migratePreserveEn
    fun introFullFor(lang: MemoryPromptLang) = if (lang == MemoryPromptLang.zh) introFullZh else introFullEn
    fun moreHintFor(lang: MemoryPromptLang) = if (lang == MemoryPromptLang.zh) moreHintZh else moreHintEn
}
